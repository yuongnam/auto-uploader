...
def deduplicate_video(input_path, output_path, steps):
    ...
    # 滤镜：亮度、对比度、饱和度、锐化
    if "filter" in steps:
        # eq=brightness=0.05:contrast=1.1:saturation=0.9, unsharp
    ...
    # BGM 替换（这里简化：只处理视频，BGM用后续步骤添加）
    # 实际需要先分离音频，再加新BGM，这里暂不实现（剪映模板会更简单）
